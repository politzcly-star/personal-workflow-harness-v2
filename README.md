...
- 新项目首次接入时安装项目根 `AGENTS.md` 协议块、建立 project profile，并默认尝试 CodeGraph / 结构索引。
...
只在聊天里说“参考 `D:\个人工作流-v2`”不算完整接入。要让新窗口持续遵守父子线程、路由和验证协议，必须把协议安装到目标项目根 `AGENTS.md`，或合并等价项目规则。

1. 读 `README.md`、`AGENTS.md`、`docs/install-hooks-upgrade.md`。
2. 把 `templates/project-agents.md` 安装或合并到目标项目根 `AGENTS.md`。
3. 用 `templates/project-profile.md` 建立项目画像。
4. 默认尝试 CodeGraph / 结构索引。
5. 如果 CodeGraph 不可用，不阻塞工作，记录 fallback：`rg` + 文件树 + 测试入口 + 依赖/调用关系手动定位。
6. 默认确认 parent-router / child-executor 是否可用；非 trivial 任务必须走 child task / child report。
7. 默认检查是否已有项目服务器 SSH alias；如果有，直接使用 `server_inspection` 做只读查询；如果没有，提示用户先在本机配置一次 alias。
8. 记录包管理器、启动命令、测试命令、构建命令、端口、主要目录、禁止修改区域、数据库边界、部署边界、常用验证命令。
...
2. 新项目先安装 `templates/project-agents.md` 到项目根 `AGENTS.md`，再用 `templates/project-profile.md`。
