...
- `.codex/agents/luna-qa.toml`, `terra-worker.toml`, `luna-verifier.toml`, and `sol-planner.toml`: current GPT-6 role files with stable compatibility IDs. Modern clients can discover standalone definitions; older clients may use explicit registrations. Only known registrations actually present in source config are merged. Definition validity, legacy target matching, tool-declared availability and successful runtime requests are separate facts; installing files does not prove provider support. Unsupported optional roles use the bounded fallback policy, not a login/provider switch.
